...
int main (int argc, char *argv[]){	
	 ...
	 **/
	printf("Parameter of the trial wave function and acceptance\n");
	for(sigma=sigma_min; sigma<sigma_max; sigma+=0.033){
		...
		for(sweep=0; sweep<SWEEPS; sweep++){
			acc_ratio += metropolis(probability, x, 1, delta);
		...
		printf("%.2lf\n", acc_ratio/SWEEPS);
		if((int)((sigma-sigma_min)/0.033) == 5)
			printf("\t\tPrinted something on file, to check the distribution.\n");
